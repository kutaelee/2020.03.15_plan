...
public class MemberController {
	 ...
	 * 
	 * mv.setId("google_"+id[0]); mv.setEmail(email);
	 * 
	 * 
	 * if (!ObjectUtils.isEmpty(ms.socialCheck(mv))) { 기존회원 로그인
	 * session.setAttribute("userseq", ms.socialCheck(mv));
	 * session.setAttribute("social",true); return true; } else { 신규가입
	 * if(ms.emailCheck(email)) { ms.socialJoin(mv); session.setAttribute("userseq",
	 * ms.socialCheck(mv)); session.setAttribute("social",true); return true; }else
	 * { return false; }
	 * 
	 * }
	 * 
	 * }
	 * 
	 * @PostMapping(value="socialusercheck") public @ResponseBody boolean
	 * socialUserCheck(HttpSession session) {
	 * if(ObjectUtils.isEmpty(session.getAttribute("social"))){ return false; }else
	 * { return true; } }
	 * 
	 * 
	 * @PostMapping(value="naverlogin") public @ResponseBody boolean
	 * naverLogin(HttpServletRequest req,HttpSession session) { String email =
	 * req.getParameter("email"); String[] id=email.split("@");
	 * mv.setId("naver_"+id[0]); mv.setEmail(email);
	 * 
	 * 
	 * if (!ObjectUtils.isEmpty(ms.socialCheck(mv))) { 기존회원 로그인
	 * session.setAttribute("userseq", ms.socialCheck(mv));
